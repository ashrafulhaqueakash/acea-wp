(function () {
    ...
    PACopyPasteHandler = {
        b: function (b, c) {
            ...
            i && jQuery.ajax({
                url: acea_cross_cp.ajax_url,
                method: "POST",
                data: {
                    nonce: acea_cross_cp.nonce,
                    action: "acea_cross_cp_import",
                    copy_content: h,
                }
            }).done(function (a) {
                if (a.success) {
        ...
        pasteAll: function (allSections) {
            jQuery.ajax({
                ...
                if (e.success) {
                    var data = e.data[0];
                    if (acea_cross_cp.elementorCompatible) {
    ...
    c.forEach(function (a, e) {
        elementor.hooks.addFilter("elements/" + c[e] + "/contextMenuGroups", function (a, f) {
            return d.push(f), a.push({
                name: "acea_" + c[e],
                actions: [{
                    name:  "acea_addons_copy",
                    title: "Acea | Copy Section",
                    icon:  "pa-dash-icon",
                    callback: function () {
                        var a = {};
                        a.eletype = "widget" == c[e] ? f.model.get("widgetType") : null, a.elecode = f.model.toJSON(), xdLocalStorage.setItem("acea-c-p-element", JSON.stringify(a)), console.log(a)
                    }
                }, {
                    name: "acea_addons_paste",
                    title: "Acea | Paste Section",
                    icon: "pa-dash-icon",
                    callback: function () {
                        xdLocalStorage.getItem("acea-c-p-element", function (a) {
                            PACopyPasteHandler.b(JSON.parse(a.value), f)
                        })
                    }
                },
                {
                    name: "acea_addons_copy_all",
                    title: "Acea | Copy All Content",
                    icon: "pa-dash-icon",
                    callback: function () {
                        ...
                        xdLocalStorage.setItem('acea-c-p-all', JSON.stringify(allSections), function (a) {
                            elementor.notifications.showToast({
                ...
                {
                    name: "acea_addons_paste_all",
                    title: "Acea | Paste All Content",
                    icon: "pa-dash-icon",
                    callback: function () {
                        var allSections = '';
                        xdLocalStorage.getItem('acea-c-p-all', function (a) {
