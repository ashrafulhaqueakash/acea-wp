<?php
/**
 * Acea class-acea-short-about.php widget class file
 *
 * @package Acea
 */
/**
 * AceaContacts widget.
 */
class AceaAbout extends WP_Widget
{
	/**
	...
	function __construct()
	{
		parent::__construct(
			'acea_about', // Base ID
			esc_html__('Acea About', 'acea'), // Name
			array('description' => esc_html__('Display Footer Short About Info', 'acea')) // Args
		);
...
?>
		<div class="acea-footer-about-wrap">
			...
			<?php if (!empty($description) && !empty($description)) : ?>
				<p class="text-acea-manatee">
				<p><?php echo wp_kses_post($description); ?></p>
			<?php endif ?>
			<div class="acea-social-icon">
	...
	?>
		<div id="fa_all_markup_wrap" style="display: none;">
			<div class="fa-all-thing-wrapper">
				<div class="themify-icon-search">
					<input type="text" class="themify_search" placeholder="<?php echo esc_attr_e('Search themify icon', 'acea'); ?>">
				</div>
				<div class="themify-icon-list-wrap">
					<ul>
						<?php
						if (function_exists('acea_get_social_icons')) :
							$themify_icons = acea_get_social_icons();
				...
				<button class="fa_tb_close button-primary"><?php esc_html_e('Insert Icon', 'acea'); ?></button>
			</div>
		</div>
		<p>
		<div class="single-widget-uploader">
			...
			<button class="sl-widget-up-btn button" data-url="<?php echo esc_url(get_site_url()); ?>"><?php echo esc_html__('Upload image', 'acea'); ?></button>
			<?php $thumnbnail = !empty($thumb) ? esc_attr($thumb) : ''; ?>
			<button <?php echo (!empty($thumnbnail)) ? 'style="display: inline-block;"' : ''; ?> class="sl-widget-media-remove button" data-url="<?php echo esc_url(get_site_url()); ?>"><?php echo esc_html__('Remove image', 'acea'); ?></button>
			<br><br>
			<?php
			if (!empty($thumnbnail)) :
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('description')); ?>"><?php esc_html_e('Description:', 'acea'); ?></label>
		...
		<div class="acea-repeater-info-wrapper" data-id-name="acea_about" data-field-id="social_links" data-placeholder="<?php esc_attr_e("Social Page Url", "acea"); ?>">
				...
				?>
					<div class="single-repeater-field-wrap">
						...
						<div class="single-repater-content">
								...
								<label for="<?php echo esc_attr($this->get_field_id('social_links') . '[' . esc_attr($index) . '][thumb-id]'); ?>">
									<a class="button thickbox_special"><span>
											...
											<?php
												...
												esc_html_e('Select an Icon', 'acea');
											}
											?>
										</span></a>
							...
							<input class="sl-repater-title-in" type="text" placeholder="<?php esc_attr_e('Social Page Url', 'acea'); ?>" name="<?php echo esc_attr($this->get_field_name('social_links') . '[' . esc_attr($index) . '][title]'); ?>" value="<?php echo (!empty($item['title'])) ? esc_attr($item['title']) : ''; ?>">
			...
			<a class="button acea-widget-add-item" data-widget-type="contact_info"><?php esc_html_e('Add Social Link', 'acea'); ?></a>
		</div>
<?php
	}
} // class AceaAbout
if (!function_exists('acea__about_widget_init')) {
	function acea__about_widget_init()
	{
		register_widget('AceaAbout');
	}
}
add_action('widgets_init', 'acea__about_widget_init');
